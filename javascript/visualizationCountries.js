...
export function drawStackedBarChart(timeseriesCountries, avgHeightForSurface)
{
    ...
    // Create scale for second Y axis (avgHeight)
    const yHeight = d3.scaleLinear()
        .domain([d3.min(avgHeight), d3.max(avgHeight)])
        .nice()
        .range([height, 0]);
    ...
    // Add line for avgHeight
    const line = d3.line()
        .x((_, i) => x(data[i].timestamp) + x.bandwidth() / 2) // Center points within bars
        .y((d, i) => yHeight(d));
    ...
    // Add points for avgHeight
    svg.selectAll(".dot")
        .data(avgHeight)
        .enter().append("circle")
        .attr("cx", (_, i) => x(data[i].timestamp) + x.bandwidth() / 2)
        .attr("cy", d => yHeight(d))
        .attr("r", 5) 
        .attr("fill", "#b0b0b0");
    ...
    svg.selectAll(".dot")
        ...
        .attr("fill", "#b0b0b0")
        .on("mouseover", function (event, d) {
            // Show tooltip with height value
            d3.select(this)
                .attr("r", 7); // Enlarge the circle on hover

            tooltip.style("opacity", 1)
                .html(`<strong>Height:</strong> ${d.toFixed(2)} cm`) // Display height value
                .style("left", (event.pageX + 10) + "px")
                .style("top", (event.pageY - 20) + "px");
        })
        .on("mousemove", function (event) {
        ...
        .on("mouseout", function () {
            // Hide tooltip and reset circle size
            d3.select(this)
    ...
    // Add right Y axis (avgHeight)
    svg.append("g")
        .attr("class", "y axis")
        .attr("transform", `translate(${width}, 0)`)
        .call(d3.axisRight(yHeight).ticks(6));
    ...
    svg.append("text")
        ...
        .text("Height (cm)")
        .style("font-size", "14px")
        .attr("transform", `rotate(90, ${width}, ${height / 2})`); // Rotazione di 90° attorno al punto (width, height / 2)
